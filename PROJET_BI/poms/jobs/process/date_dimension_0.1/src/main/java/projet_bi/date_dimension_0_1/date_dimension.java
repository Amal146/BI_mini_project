...
package projet_bi.date_dimension_0_1;
...
public class date_dimension implements TalendJob {
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(date_dimension.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class date_dimensionStruct implements routines.system.IPersistableRow<date_dimensionStruct> {
		final static byte[] commonByteArrayLock_PROJET_BI_date_dimension = new byte[0];
		static byte[] commonByteArray_PROJET_BI_date_dimension = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROJET_BI_date_dimension.length) {
					if (length < 1024 && commonByteArray_PROJET_BI_date_dimension.length == 0) {
						commonByteArray_PROJET_BI_date_dimension = new byte[1024];
					} else {
						commonByteArray_PROJET_BI_date_dimension = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROJET_BI_date_dimension, 0, length);
				strReturn = new String(commonByteArray_PROJET_BI_date_dimension, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_PROJET_BI_date_dimension, 0, length);
				strReturn = new String(commonByteArray_PROJET_BI_date_dimension, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROJET_BI_date_dimension) {
	...
	public void tFileInputDelimited_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (execStat) {
					runStat.updateStatOnConnection(resourceMap, iterateId, 0, 0, "date_dimension");
				}
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File(
						"C:/Users/adelj/TOS_BD-20211109_1610-V8.0.1/workspace/dateDimension.csv")).getAbsolutePath()
								.replace("\\", "/");
...
// # Output table : 'date_dimension'
								date_dimension_tmp.date_id = TalendDate.formatDate("yyyyMMdd",
										row1._order_purchase_timestamp);
								;
								date_dimension_tmp.year = TalendDate.formatDate("yyyy", row1._order_purchase_timestamp);
								;
								date_dimension_tmp.month = TalendDate.formatDate("MM", row1._order_purchase_timestamp);
								;
								date_dimension_tmp.day = TalendDate.formatDate("dd", row1._order_purchase_timestamp);
								;
...
// Start of branch "date_dimension"
							if (date_dimension != null) {
								...
								if (date_dimension.date_id != null) {
									sb_tFileOutputDelimited_1.append(date_dimension.date_id);
								}
								sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
								if (date_dimension.year != null) {
									sb_tFileOutputDelimited_1.append(date_dimension.year);
								}
								sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
								if (date_dimension.month != null) {
									sb_tFileOutputDelimited_1.append(date_dimension.month);
								}
								sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
								if (date_dimension.day != null) {
									sb_tFileOutputDelimited_1.append(date_dimension.day);
								}
...
// ###############################      
				...
				if (execStat) {
					runStat.updateStat(resourceMap, iterateId, 2, 0, "date_dimension");
				}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = date_dimension.class.getClassLoader()
					.getResourceAsStream("projet_bi/date_dimension_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = date_dimension.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : date_dimension");
		}
...
/************************************************************************************************
 * 61609 characters generated by Talend Open Studio for Big Data on the 18
 * janvier 2024, 11:39:21 PM CET
 ************************************************************************************************/